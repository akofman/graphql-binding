...
function extractQuerySelectionSet(query, typeName, schema) {
  ...
  var errors = graphql_1.validate(schema, document).filter(function(e) {
    return e.message.match(/Fragment ".*" is never used./) === null
  })
  if (errors.length > 0) {
    throw errors
  }
...
function makeSubInfo(info, path, fragment) {
  ...
  while (fieldsToTraverse.length > 0) {
    ...
    if (!graphql_1.isObjectType(currentType)) {
      throw new Error(
        "Can't get subInfo for type " +
          currentType.toString() +
          ' as needs to be a GraphQLObjectType',
      )
    }
    ...
    var suitableSelection = currentSelectionSet.selections.find(function(
      selection,
    ) {
      return (
    ...
    if (!suitableSelection) {
      ...
      currentSelectionSet = currentSelectionSet.selections.reduce(function(
        acc,
        curr,
      ) {
        if (acc) {
