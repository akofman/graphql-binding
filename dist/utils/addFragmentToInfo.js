...
function addFragmentToInfo(info, fragment) {
  ...
  if (returnType instanceof graphql_1.GraphQLScalarType) {
    throw new Error(
      'Can\'t add fragment "' +
        fragment +
        '" because return type of info object is a scalar type ' +
  ...
  if (
    ...
    ast.definitions[0].typeCondition.name.value !== deepReturnType.toString()
  ) {
    throw new Error(
      'Type ' +
        ast.definitions[0].typeCondition.name.value +
        " specified in fragment doesn't match return type " +
